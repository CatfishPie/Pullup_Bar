function MoveStraight (Inches: number) {
    ...
    Gyro_Starting_Angle = sensors.gyro1.angle()
    kp = 0.5
    target_angle = 0
    Rotations_to_Move = Inches / 5.35 + 0.1
    NewRotations = 0
    if (Inches > 0) {
        while (motors.largeA.angle() / 360 <= Rotations_to_Move) {
            // or is it + ?
            error = Gyro_Starting_Angle - sensors.gyro1.angle() - target_angle
            motors.largeAD.steer(error * kp, 50)
        }
    } else {
        while (motors.largeA.angle() / 360 >= Rotations_to_Move) {
            // or is it + ?
            error = Gyro_Starting_Angle - sensors.gyro1.angle() - target_angle
            motors.largeAD.steer(error * (0 - kp), -50)
        }
    }
    motors.largeAD.stop()
}
...
let error = 0
let NewRotations = 0
let Rotations_to_Move = 0
let target_angle = 0
let kp = 0
let Gyro_Starting_Angle = 0
checkGyroHappiness()
FindLineClose()
LineFollowRight(27, 30)
TurnDegrees(90)
pause(100)
motors.largeAD.run(-50, 2, MoveUnit.Seconds)
MoveStraight(24)
